...
export async function antigravityCreateNotionPage({ databaseId, title, status, content, properties } = {}) {
  ...
  return notionRest('/pages', 'POST', {
    ...
    properties: { title: [{ text: { content: title } }], ...(status ? { Status: { select: { name: status } } } : {}), ...(content ? { Content: { rich_text: [{ text: { content } }] } } : {}), ...(properties && typeof properties === 'object' ? properties : {}) },
  });
}
...
export async function antigravityRunAutomation({ name, databaseId, input } = {}) {
  if (!name) throw new Error('name is required');
  try {
    return await mcpPost('/automations/run', { name, database_id: databaseId || null, workspace_id: antigravityConfig().workspaceId || null, input: input && typeof input === 'object' ? input : {} });
  } catch {
    const page = await antigravityCreateNotionPage({ databaseId, title: `Automation run: ${name}`, status: 'Queued', content: typeof input === 'string' ? input : JSON.stringify(input || {}) });
