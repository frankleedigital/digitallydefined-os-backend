#!/usr/bin/env python3
"""
Brevo IMAP Inbox Triage — reads inbox, scores by urgency, surfaces only what clears the bar.
Uses the pre-existing brevo_triage.py logic from DigitallyDefinedClean/scripts/.
"""
import imaplib
import email
...
def triage_email(sender, subject, body, date_str):
    ...
    is_family = any(k in sender_lower for k in family_keywords) or any(k in subject_lower for k in family_keywords)
    
    manager_indicators = ["manager", "director", "executive", "ceo", "cto", "vp ", "head of", "boss", "supervisor"]
    is_manager = any(k in sender_lower for k in manager_indicators)
    
    deadline_keywords = ["deadline", "due", "by ", "before ", "this week", "this month",
    ...
    reply_indicators = ["?", "please", "help", "could you", "can you", "would you",
...
def main():
    ...
    if not imap_pass:
        ...
        print("Username: francesca@digitallydefined.online")
        sys.exit(1)
